...
- Initialize and track task-focused do.md workflow on a work branch.
- Pull work items from backlog providers into current task context.
- Manage local environment bootstrap for script paths and .env.local conventions.
- Move uncommitted changes to a remote safety branch.
- Transform delimited and line-oriented value lists.

## Candidate Activities To Script Next

- One-command bootstrap for branch + do.md + initial backlog pop.
- Environment health checks for required CLI tools and variables.
- Guided task archive workflow for completed story summaries.
