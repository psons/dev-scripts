#!/usr/bin/env python3
"""Goal Blotter data model and markdown parsing utilities."""
...
def parse_stories_from_markdown(
    ...
    current_story_status: TaskStatus | None = None
    ...
    def finalize_task() -> None:
        ...
        task_id = f"task-{current_story_index}-{current_task_index}-{_hash8(task_name)}"
    ...
    def finalize_story() -> None:
        ...
        story_id = f"story-{current_story_index}-{_hash8(story_name)}"
        stories.append(
            Story(
                id=story_id,
                name=story_name,
